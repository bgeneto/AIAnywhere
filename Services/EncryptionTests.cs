...
namespace AIAnywhere.Tests
{
    ...
    public static class PortableEncryptionTests
    {
        public static void RunTests()
        {
            ...
            Console.WriteLine($"Encryption/Decryption Test: {(original == decrypted ? "PASSED" : "FAILED")}");
            Console.WriteLine();
            ...
            Console.WriteLine($"IsEncrypted detection: Original={isOriginalEncrypted}, Encrypted={isEncryptedDetected}");
            Console.WriteLine($"IsEncrypted test: {(!isOriginalEncrypted && isEncryptedDetected ? "PASSED" : "FAILED")}");
            Console.WriteLine();
            ...
            Console.WriteLine($"Masking test: {(masked.StartsWith("sk-t") && masked.Contains("•") ? "PASSED" : "FAILED")}");
            ...
            Console.WriteLine($"Full masking test: {(fullyMasked.All(c => c == '•') ? "PASSED" : "FAILED")}");
              Console.WriteLine("\nAll portable encryption tests completed!");
            Console.WriteLine("✅ This encryption is PORTABLE - the same config.json will work on different machines!");
        }
    }
}
